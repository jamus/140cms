		...
		//echo $decode[$i][text]."<br>";
			if (strpos($decode[$i][text],'#'.$keyword.'') !== false) {
    			//echo $decode[$i][text]."<br>";
					...
					//echo '$rightnow: ' . $rightnow . '<br>';
    				//echo strtotime($rightnow), "<br><br>";
    				
    				echo 'Was posted ' . timeBetween($tweet_stamp, $rightnow).'<br>';
    				
    				$tweetedtime = timeBetween($tweet_stamp, $rightnow);
					...
					<h3><a href="http://twitter.com/#!/'.$twitteruser.'/statuses/'.$id.'" target="_blank" >' . $tweetedtime .'</a></h3>
					<p >'. $tweet .'</p>
					<ul>' . $tags . '</ul>
					</article>';
